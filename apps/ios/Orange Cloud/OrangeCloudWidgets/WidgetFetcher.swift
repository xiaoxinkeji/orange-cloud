...
//  OrangeCloudWidgets
//
//  Widget 自取数：用共享钥匙串里的有效 access_token 直查 GraphQL（单 Zone 24h + 前一窗口）。
//  token 过期/缺失或请求失败时返回 nil，调用方回退 App 写入的快照。
//

import Foundation

nonisolated enum WidgetFetcher {
    ...
    /// 拉取单 Zone 最新 24h 指标；任何失败返回 nil（回退快照）
    static func freshZoneMetrics(zoneId: String, name: String) async -> WidgetZoneMetrics? {
