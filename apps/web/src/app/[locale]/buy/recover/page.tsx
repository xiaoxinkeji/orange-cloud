...
import RecoverCodeForm from "@/components/RecoverCodeForm";
import { getBuyContent } from "@/lib/buy/content";

export const metadata: Metadata = { robots: { index: false, follow: false } };

export default async function RecoverCodePage({ params }: { params: Promise<{ locale: string }> }) {
			...
			<section className="sky-band band-dawn dawn-glow min-h-screen overflow-hidden">
				...
				<div className="relative mx-auto max-w-[560px] px-6 pb-24 pt-28 lg:pt-36">
					<Reveal index={0}>
						...
						<p className="mt-4 text-[15px] leading-relaxed t-secondary">{c.recover.sub}</p>
					</Reveal>
					<Reveal index={1}>
						<div className="mt-8">
							<RecoverCodeForm s={c.recover} />
						</div>
					</Reveal>
				</div>
				<SiteFooter />
