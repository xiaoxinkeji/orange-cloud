...
import type { RecoverStrings } from "@/lib/buy/content";
...
export default function RecoverCodeForm({ s }: { s: RecoverStrings }) {
	...
	return (
		<form onSubmit={submit} className="glass r-island p-7 sm:p-8">
			<label className="block text-[13px] font-medium t-primary" htmlFor="rc-email">
				{s.emailLabel}
			</label>
			<input id="rc-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder={s.emailPlaceholder} autoComplete="email" className="mt-2" style={fieldStyle} />
