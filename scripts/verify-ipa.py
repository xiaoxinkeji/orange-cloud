#!/usr/bin/env python3
"""
IPA Integrity Validator - catches missingEndOfCentralDirectoryRecord and other ZIP corruption.
...
def check_contamination(names: list):
    ...
    if contaminants:
        raise IPAError(
            ...
            f"  {contaminants[:5]}\n"
            f"  Fix: repack with Python zipfile or zip -r output.ipa Payload -x '__MACOSX/*' '*.DS_Store' '._*'"
        )
...
def verify_ipa(ipa_path: str) -> tuple[bool, str]:
    ...
    try:
        ...
        print("=== PASS - IPA is valid for signing ===")
        return True, sha
